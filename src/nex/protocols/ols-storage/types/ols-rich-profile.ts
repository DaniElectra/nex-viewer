...
export default class OLSRichProfile extends DDLClass {
	...
	private deaths = new Float();
	private rank_deaths = new UInt32();
	private jumps = new Float();
	private rank_jumps = new UInt32();
	private kills = new Float();
	private rank_kills = new UInt32();
	...
	public extractFrom(stream: NEXByteStream): void {
		...
		this.deaths.extractFrom(stream);
		this.rank_deaths.extractFrom(stream);
		this.jumps.extractFrom(stream);
		this.rank_jumps.extractFrom(stream);
		this.kills.extractFrom(stream);
		this.rank_kills.extractFrom(stream);
	...
	public toJSON(): any {
		...
		json.__fields.deaths = this.deaths;
		json.__fields.rank_deaths = this.rank_deaths;
		json.__fields.jumps = this.jumps;
		json.__fields.rank_jumps = this.rank_jumps;
		json.__fields.kills = this.kills;
		json.__fields.rank_kills = this.rank_kills;
